...
def extend_data(X,prev_frame,prev_prev_frame,prev_prev_flag):
    X = np.concatenate((X,X), axis=0)
    X = np.concatenate((X,X), axis=0)
    # X = np.concatenate((X,X), axis=0)
    # X = np.concatenate((X,prev_frame), axis=0)
    X = np.concatenate((X,prev_frame), axis=0)
    if prev_prev_flag == 1:
        X = np.concatenate((X, prev_prev_frame), axis=0)
    return X
...
class DPMeans(KMeans):
    ...
    def __init__(
        ...
        *,
        ...
        self.curr_frame = 0
        self.prev_frame = 0
        self.prev_prev_frame = 0
        self.prev_flag = 0
        self.prev_prev_flag = 0
        self.clusters_names = []
        self.last_clusters_indexs_size = 0
        self.cluster_index = 0
        self.data = 0

    def update_clusters_for_streaming(self,X):

        # #giving names to new clusters
        # if self.n_clusters > self.last_clusters_indexs_size:
        #     diff = self.n_clusters - self.last_clusters_indexs_size
        #     self.clusters_names += [self.cluster_index+i for i in range(diff) ]
        #     self.cluster_index += diff

        data_size = len(X)
        y_dpmeans = self.predict(self.data)
        counters = [0 for i in self.cluster_centers_]
        for label in y_dpmeans:
            counters[label]+=1

        deletion_sum=0
        for index,value in enumerate(counters):
            if value < data_size//100:
                self.cluster_centers_=np.delete(self.cluster_centers_,index-deletion_sum,axis=0)
                # self.clusters_names.pop(index-deletion_sum)
                self.n_clusters-=1
                deletion_sum +=1

        # self.last_clusters_indexs_size = self.n_clusters
        # return self.clusters_names
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        data_size = len(X)
        self.curr_frame = np.copy(X)
        if (self.prev_flag == 1 and self.prev_prev_flag == 1):
            #foo - extends dataset acorrding to previous centers
            X = extend_data(X,self.prev_frame,self.prev_prev_frame,prev_prev_flag=1)
        elif(self.prev_flag == 1): #if there is no prev_prev
            X = extend_data(X,self.prev_frame,prev_prev_frame=0,prev_prev_flag=0)
            self.prev_prev_flag = 1
        else:
            self.prev_flag = 1
